...
public static class AssessmentMappings
{
        ...
        AssessmentStatus assessmentStatus = AssessmentStatus.Draft)
    {
        ...
        return new Assessment
        {
            ...
            SubjectIds = (request.SubjectIds ?? []).Where(subjectId => subjectId != Guid.Empty).Distinct().ToArray(),
            TopicIds = (request.TopicIds ?? []).Where(topicId => topicId != Guid.Empty).Distinct().ToArray(),
    ...
    public static AssessmentRecord ToRecord(this Assessment assessment)
    {
        return new AssessmentRecord(
            ...
            assessment.SubjectIds,
            assessment.SubjectNames,
            assessment.SubjectName,
            assessment.SubjectName,
            assessment.TopicIds,
            assessment.TopicNames,
